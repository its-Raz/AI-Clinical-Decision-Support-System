...
def extract_latest_user_query(state: dict) -> str:
    """
    Extracts the latest user query from the global AgentState messages.
    """
    messages = state.get("messages", [])

    # Iterate backwards to find the most recent user message
    for msg in reversed(messages):
        # 1. Handle if the message is a standard dictionary
        if isinstance(msg, dict):
            role = msg.get("role") or msg.get("type")
            if role in ["user", "human"]:
                return msg.get("content", "")

        # 2. Handle if the message is a LangChain BaseMessage (e.g., HumanMessage)
        elif hasattr(msg, "type") and msg.type == "human":
            return msg.content

    # Fallback if no user message is found
    return "Could not find a user query in the message history."